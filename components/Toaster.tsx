  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex w-80 max-w-[90vw] flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`rounded-2xl px-4 py-3 shadow-lg ${variantClass[(t.variant ?? 'default') as Variant]}`}
          role="status"
          aria-live="polite"
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="shrink-0 rounded-md/50 px-2 text-sm/none opacity-80 hover:opacity-100"
