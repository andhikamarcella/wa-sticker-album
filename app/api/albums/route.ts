...
export async function GET(request: NextRequest) {
  ...
  if (!isSupabaseConfigured()) {
    ...
    const items = filtered.map(mapMockAlbumToListItem).sort(sortByUpdatedAtDesc);
...
export async function POST(request: NextRequest) {
  ...
  const slugResult = await generateUniqueSlug(supabase, slugBase);
  ...
  if (!insertResult.ok) {
    ...
    return NextResponse.json({ error: insertResult.error }, { status });
  }

  return NextResponse.json({
    id: insertResult.album.id,
    name: insertResult.album.name,
    slug: insertResult.album.slug,
    visibility: insertResult.album.visibility,
    updatedAt:
      insertResult.album.updated_at ?? insertResult.album.created_at ?? new Date().toISOString(),
  });
}
...
async function insertAlbum(
  ...
  if (!data) {
    return { ok: false as const, error: 'Failed to create album' };
  }

  return { ok: true as const, album: data as AlbumRow };
}
