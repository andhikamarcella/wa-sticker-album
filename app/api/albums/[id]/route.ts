...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (existingError) {
      if (shouldUseMockFromSupabaseError(existingError)) throw new SupabaseSchemaMissingError(existingError.message);
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!album) return NextResponse.json({ error: 'Album not found' }, { status: 404 });
    if (album.owner_id !== user.id) return NextResponse.json({ error: 'Only the owner can delete this album' }, { status: 403 });
